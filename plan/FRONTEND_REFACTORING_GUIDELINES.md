...
| P0 | **个人安全中心** | `/auth/email/*` `/auth/2fa/*` `/auth/password` `/auth/username` | 1 天 | [待分配] | ✅ 已完成 | 邮箱绑定/2FA/恢复码/修改密码/修改用户名四 Tab；API Key 管理独立在 API 文档页；详见 docs/security-page.md |
| P0 | **仪表盘（Dashboard）** | `/host/stats`(admin)/`/self/quota`(user) | 2 天 | [待分配] | ✅ 已完成 | SSE 实时推送；管理员含理论最大量双进度条 |
| P1 | **虚拟机列表** | `/vm/list` / `/self/vms` + SSE | 4 天 | [待分配] | ✅ 已完成 | 缓存优先 + SSE 常驻实时刷新；表格/卡片双视图；列头点击排序；状态/操作纯图标 + Tooltip；批量电源；删除/备注/分组/制作模板/重装/迁移弹窗；详情页随后续迭代 |
...
| P3 | **模板管理** | `/template/*` | 4 天 | [待分配] | ✅ 已完成 | 模板族树形渲染/删除三种模式/分片上传；详见 docs/template-page.md |
