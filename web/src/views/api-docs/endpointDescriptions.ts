...
export const endpointDescriptions: Record<string, EndpointDescription> = {
  ...
  'POST /vm/:name/disk': { summary: '新增磁盘', body: 'JSON: size_gb, bus, storage_pool_id, path' },
  'POST /vm/:name/disk/:dev/resize': { summary: '扩容磁盘', body: 'JSON: size_gb' },
  'PUT /vm/:name/disk/:dev/bus': { summary: '修改磁盘总线类型', body: 'JSON: bus(virtio/scsi/sata/ide)' },
  'POST /vm/:name/disk/attach': { summary: '挂载已有磁盘文件', body: 'JSON: path, bus' },
  'POST /vm/:name/disk/import': {
    summary: '为已有 VM 导入磁盘文件',
    body: 'JSON: disk_path/disk_file, disk_source_type, copy_disk, bus, storage_pool_id',
  },
  ...
  'POST /vm/:name/password/reset': {
    summary: '离线重置虚拟机系统密码',
    body: 'JSON: username, password, guest_type(linux/windows/fnos)',
  },
