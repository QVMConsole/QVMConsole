...
export const endpointGroups = [
  ...
  },
  {
    ...
    endpoints: [
      ...
      ep('POST', '/vm/batch-clone', '批量克隆虚拟机', { body: 'JSON: prefix(名称前缀), start_num(起始编号), count(创建数量), template, template_type, clone_mode(linked/full), vcpu, ram, disk_size, hostname(可选), user(新建用户), password, freeze, template_root_pass, template_user, video_model(支持none禁用虚拟显示), spice_enabled(bool,是否启用SPICE显示协议,不传=回退全局默认), disk_bus, cpu_topology_mode, first_boot_reboot_mode, uefi；count>1 时不允许 host_devices', notes: [elastic] }),
