...
| 全局策略 | 虚拟网桥、虚拟机网段、出站/入站区域限制（TextSwitch + 区域多选）、禁用 VM IPv6、拦截动作（reject/drop）、白名单 CIDR（每行一个） |
...
- `web/src/features/vm-form/sections/TextSwitch.tsx`：带文字开关（Switch + 右侧状态文字），样式在 firewall.css 内置同名类避免依赖 vm-form 样式加载时机
...
3. **开关组件统一**：旧版 `el-switch` 内嵌中文（active-text/inactive-text）在窄空间会竖排，新版统一使用共享组件 TextSwitch（Switch + 右侧外部状态文字）。
