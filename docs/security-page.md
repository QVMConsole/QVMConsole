...
> 注意：API Key 管理不在本页，已独立到侧边栏「API 文档」页。

## 功能总览

共 4 个标签页，与旧前端安全设置对话框一一对应：
...
- `web/src/api/auth.ts`：新增安全中心全部接口（changePassword / changeUsername / sendEmailCode / bindEmail / setup2FA / enable2FA / disable2FA / regenRecoveryCodes）与类型（TotpSetupInfo / RecoverySetup / ApiResponseWithRecovery 等）
