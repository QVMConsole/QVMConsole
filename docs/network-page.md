...
| 交换机表单 | 管理员可选所属用户与目标网桥；选中桥接网桥时隐藏网段配置，显示桥接 VLAN ID 与桥接安全三开关（混杂/MAC 更改/伪传输）；端口安全总开关开启时可配置直通桥 IPv6 防护和可信前缀；网段/网关创建后不可改，留空自动分配；流量/带宽配额按用户剩余配额限制上下限，编辑时可为负表示归还配额 |
