...
| 分区行 | 树形扁平化缩进渲染；徽章（系统分区/默认/已启用/存在数据/只读）；自绘细条 capbar（按使用率变色）；操作同磁盘（配置外露 + ⋯ 下拉） |
| VM 占用展示 | 存储池分区下方显示虚拟机磁盘占用：总数/实际占用/虚拟配置总量；每台 VM 显示名称和实际/虚拟大小；进度条按实际占用率变色（≥70 橙 / ≥90 红） |
...
│   ├── DiskCard.tsx                 # 磁盘/VG 卡片（空间分配条 + VM 虚拟总量进度条 + 引导区 + 徽章 + 图标）
...
新增文件：`server/service/storage/pool/vm_usage_inject.go`

- `getAllVMDiskUsage()`：遍历所有虚拟机，通过 `virsh dumpxml` 获取磁盘路径，再用 `qemu-img info --output=json` 获取虚拟配置大小和实际占用大小
- `VMDiskUsageInfo`：单台虚拟机的磁盘使用信息（名称/路径/虚拟大小/实际大小/挂载点）
- `injectVMUsageIntoPools()`：将 VM 磁盘使用统计注入存储池树，为每个分区节点填充 `VmUsageList`、`VmTotalVirtual`、`VmTotalActual` 字段
